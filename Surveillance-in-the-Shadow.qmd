...
To bridge the 1930s HOLC geography with present-day analyses, we crosswalk HOLC grades onto modern (2020) census tracts. Rather than re-deriving the overlay ourselves, we use the published **Mapping Inequality v3 → 2020 Tract crosswalk** from American Panorama [6], which provides — for every HOLC area that intersects a U.S. census tract — the share of that tract covered by the area (`pct_tract`). We aggregate those shares per (tract, grade), retain the full share vector so tracts that straddle multiple grades aren't forced into a binary classification, and assign a single **modal grade** — the grade covering the largest share of the tract.
...
# Load the pre-computed Mapping Inequality v3 crosswalk and restrict to Chicago, IL.
# `pct_tract` is the share of each 2020 tract covered by a given HOLC area.
mi_crosswalk <- st_read("data/MIv3Areas_2020TractCrosswalk.geojson", quiet = TRUE) |>
  st_drop_geometry() |>
  filter(city == "Chicago", state == "IL", !is.na(grade)) |>
  select(GEOID, grade, pct_tract)

# Sum shares per (tract, grade) — a tract may overlap multiple HOLC areas of the same grade.
grade_area <- mi_crosswalk |>
  group_by(GEOID, grade) |>
  summarise(share = sum(pct_tract), .groups = "drop") |>
  as.data.frame()

# Pivot long → wide via base reshape (one column per grade share)
grade_shares <- reshape(grade_area, idvar = "GEOID", timevar = "grade", direction = "wide")
names(grade_shares) <- sub("share\\.", "share_", names(grade_shares))

# Modal (largest-share) grade per tract — sort then keep first per GEOID
ord <- order(grade_area$GEOID, -grade_area$share)
